...
Primo gruppo di annotazioni: dialogue act. Per selezionare uno o più dialogue act basta cliccare sul nome dell’act corrispondente e inserire i dati degli slot richiesti.
...
### Sys-greet: usata per annotare le utterance (= frasi) in cui il sistema saluta l’utente
...
Distinguiamo gli slot in generali e in slot che costituiscono gli attributi della nostra entità job-offer.

Slot generali

Global slot. Questo tipo di annotazione  permette di marcare l’intero dialogo invece del singolo turno e serve per annotare se il sistema ha trovato o meno un’offerta lavorativa adatta all’utente. I valori che può assumere sono: 


positive:stringa da inserire se l’utente, alla fine del dialogo, è soddisfatto dell’offerta di lavoro proposta dal sistema.


negative: stringa da inserire se l’utente NON è soddisfatto dell’offerta di lavoro proposta. 
...
turn_ref: da usare qualora ci sia una sovrapposizione di domanda/risposta tra utente/sistema. Ad esempio:
...
IMPORTANTE: come valore degli slot selezionare il minor numero di parole possibili (idealmente, non più di 3-4). E’ possibile tuttavia selezionare separatamente più elementi per la stessa frase.
Esempio:
...
IMPORTANTE 2: Cercare di ridurre al minimo indispensabile di inserire valore “none” all’interno degli slot, perché non sono informativi. 
